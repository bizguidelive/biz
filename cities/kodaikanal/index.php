...
// Subdomain: kodai.bizguide.in  |  Subfolder: bizguide.in/cities/kodaikanal
define('CITY_URL', defined('IS_SUBDOMAIN') && IS_SUBDOMAIN
    ? 'https://' . ($_SERVER['HTTP_HOST'] ?? $citySlug . '.bizguide.in')
    : BASE_URL . '/cities/' . $citySlug
);
...
$router->get( "$base/set-password",            'AuthController',    'setPassword');
$router->post("$base/set-password",            'AuthController',    'savePassword');
$router->get( "$base/logout",                  'AuthController',    'logout');

// ── User (authenticated) ──────────────────────────────────────
$router->get( "$base/dashboard",               'UserController',    'dashboard');
$router->post("$base/profile/update",          'UserController',    'updateProfile');
...
$router->get("$base/listing/{slug}",           'ListingController', 'show');
